...
When the tools `presentation_brief` and `presentation_form` are in your tool list, the form is served by the KLERQ MCP server itself and rendered by the chat client. Everything else in this file still applies (what data to gather, how to prefill from the brief, what the answer messages look like, how to create the record); only the rendering changes:
